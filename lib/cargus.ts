...
function normalizeStatus(text: string): CargusStatus { const value = text.toLocaleLowerCase("ro"); if (/(livrat|livrată|livrata|predat destinatar|predată destinatar|confirmat|delivered)/i.test(value)) return "delivered"; if (/(în livrare|in livrare|livrare|spre destinatar|expediat|expediată|preluat|preluată|tranzit|în tranzit|in tranzit|plecat|plecare|sosit|sosire|ajuns|intrare în depozit|intrare in depozit|ieșire din depozit|iesire din depozit|sortare|centru|pudo)/i.test(value)) return "sent"; return "preparing"; }
