...
export const orderUpdateSchema = z.object({
  ...
  status: z.enum(["code", "address", "not_sent", "preparing", "sent", "delivered", "shipping"]).optional(),
  awbStatus: z.string().max(240).nullable().optional(),
  awbStatusDescription: z.string().max(500).nullable().optional(),
  awbLastCheckedAt: z.coerce.date().nullable().optional(),
});

export type OrderRecord = {
  code: string;
  status: "code" | "address" | "not_sent" | "preparing" | "sent" | "delivered" | "shipping";
  address: string;
  awb: string;
  awbStatus?: string | null;
  awbStatusDescription?: string | null;
  awbLastCheckedAt?: Date | null;
  orderPrice: number;
...
export async function createOrder(code: string) { const now = new Date(); const order: OrderRecord = { code, status: "code", address: "", awb: "", awbStatus: null, awbStatusDescription: null, awbLastCheckedAt: null, orderPrice: 0, shippingPrice: 0, shippingPaid: false, createdAt: now, updatedAt: now }; await (await ordersCollection()).insertOne(order); return order; }
