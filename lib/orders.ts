...
export async function createOrder(code: string) { const now = new Date(); const order: OrderRecord = { code, status: "code", customerName: "", customerPhone: "+40", address: "", shippingMethod: "ship_go", shippingMethodLocked: false, lockerName: null, awb: "", awbStatus: null, awbStatusDescription: null, awbLastCheckedAt: null, awbEvents: [], awbRoute: [], orderPrice: 0, shippingPrice: 0, shippingOriginalPrice: 0, shippingPaid: false, createdAt: now, updatedAt: now }; await (await ordersCollection()).insertOne(order); return order; }
